	...
	<jstl:choose>
		...
		<acme:input-moment code="assistance-agent.tracking-log.form.label.lastUpdateMoment" path="lastUpdateMoment" readonly="true"/>
    		<jstl:if test="${draftMode == true}">
    			<acme:submit code="assistance-agent.tracking-log.form.button.delete" action="/assistance-agent/tracking-log/delete?claimId=${claimId}&trackingLogId=${id}"/>
    			<acme:submit code="assistance-agent.tracking-log.form.button.update" action="/assistance-agent/tracking-log/update?claimId=${claimId}&trackingLogId=${id}"/>
    			<acme:submit code="assistance-agent.tracking-log.form.button.publish" action="/assistance-agent/tracking-log/publish?claimId=${claimId}&trackingLogId=${id}"/>
    		</jstl:if>
		</jstl:when>
